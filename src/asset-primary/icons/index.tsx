...
export const Icon = (props: Partial<CustomIconComponentProps> & { asset: AssetRow }) => {
  ...
  if (PrimaryAssetType.Category.getTypes(['vibration']).includes(type)) {
    return <AntIcon component={() => <MotorSvg {...commonProps} />} />;
  } else if (PrimaryAssetType.Category.getTypes(['corrosion']).includes(type)) {
    return <AntIcon component={() => <CorrosionSvg {...commonProps} />} />;
  } else if (PrimaryAssetType.Category.getTypes(['device']).includes(type)) {
    return <AntIcon component={() => <DeviceSvg {...commonProps} />} />;
  } else if (PrimaryAssetType.Flange === type) {
    return <AntIcon component={() => <FlangeSvg {...commonProps} />} />;
  } else if (PrimaryAssetType.Tower === type) {
