    ...
    description: {
      ...
      children:
        status.key === 0 && types.length === 0
          ? intl.get('NONE')
          : types.map((type) => intl.get(FaultType.get(type.key).label)).join(separator)
    },
      ...
      children:
        status.key === 0 && types.length === 0
          ...
          : faults.map(({ type, confidence, axis }) => {
              const typeLabel = intl.get(FaultType.get(type).label);
...
export const flattenFaultTypes = (types: number[]) => {
  return Array.from(new Set(types)).map(FaultType.get);
};
